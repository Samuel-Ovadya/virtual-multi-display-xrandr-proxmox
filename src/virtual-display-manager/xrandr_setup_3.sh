...
# Calculate new framebuffer size (3x width of Virtual-1)
new_fb_width=$((width * 3))
new_fb_height=$height

echo "Calculated framebuffer size: ${new_fb_width}x${new_fb_height}"

# Add modes for Virtual-2 and Virtual-3 with the same resolution as Virtual-1
cmd="xrandr --addmode Virtual-2 ${width}x${height}"
echo "Executing: $cmd"
eval "$cmd"

cmd="xrandr --addmode Virtual-3 ${width}x${height}"
echo "Executing: $cmd"
eval "$cmd"

# Set the framebuffer to accommodate all three displays
cmd="xrandr --fb ${new_fb_width}x${new_fb_height} --output Virtual-1 --panning ${width}x${height}/${new_fb_width}x${new_fb_height}"
echo "Executing: $cmd"
eval "$cmd"
...
cmd="xrandr --output Virtual-3 --mode ${width}x${height} --right-of Virtual-2"
echo "Executing: $cmd"
eval "$cmd"

echo "Virtual triple display setup complete!"
